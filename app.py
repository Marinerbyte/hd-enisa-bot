...
# ... (Poora Flask App code same rahega) ...
LOGIN_TEMPLATE = """
<!DOCTYPE html>
...
def get_ai_response(user_message, sender, room_id):
    ...
    try:
        ...
        behavior_response = supabase.table('user_behaviors').select('behavior_prompt').eq('username', sender_lower).single().execute()
        ...
        if behavior_response.data:
            ...
            room_pers_response = supabase.table('room_personalities').select('personality_name').eq('room_id', str(room_id)).single().execute()
            ...
            log_msg = f"🤖 Using room personality '{personality_name_to_use}'" if room_pers_response.data else f"🤖 Using default personality '{personality_name_to_use}'"
            logging.info(f"{log_msg} for room {room_id}")

            personality_response = supabase.table('personalities').select('prompt', 'style').eq('name', personality_name_to_use).single().execute()
            
            if not personality_response.data:
                personality_response = supabase.table('personalities').select('prompt', 'style').eq('name', Config.DEFAULT_PERSONALITY).single().execute()
        ...
        memory_response = supabase.table('conversation_memory').select('history').eq('username', sender_lower).single().execute()
...
def handle_master_command(sender, command, args, room_id):
    try:
        if command == 'adb':
            ...
            if not args:
                room_pers_res = supabase.table('room_personalities').select('personality_name').eq('room_id', str(room_id)).single().execute()
            ...
            supabase.table('personalities').upsert({'name': name, 'prompt': prompt, 'style': 'none'}).execute()
...
def process_command(sender, room_id, message_text):
    # This function is now run in a thread, so it can have blocking calls
...
def on_close(ws, close_status_code, close_msg):
    ...
    if bot_state.stop_bot_event.is_set():
        ...
        # Don't auto-reconnect here if using a web panel to prevent loops.
        # The web panel or uptime service will restart the bot.
